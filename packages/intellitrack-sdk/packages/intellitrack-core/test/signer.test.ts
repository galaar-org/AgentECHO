...
describe('Signer', () => {
  ...
  it('should generate signature with correct headers', async () => {
    ...
    expect(result.headers).toHaveProperty('x-intellitrack-key-id', keyId);
    expect(result.headers).toHaveProperty('x-intellitrack-ts', timestamp.toString());
    expect(result.headers).toHaveProperty('x-intellitrack-signature');
    expect(typeof result.headers['x-intellitrack-signature']).toBe('string');
    expect(result.headers['x-intellitrack-signature'].length).toBeGreaterThan(0);
  });
  ...
  it('should generate consistent signatures for same input', async () => {
    ...
    expect(result1.headers['x-intellitrack-signature']).toBe(
      result2.headers['x-intellitrack-signature']
    );
  });
  ...
  it('should generate different signatures for different timestamps', async () => {
    ...
    expect(result1.headers['x-intellitrack-signature']).not.toBe(
      result2.headers['x-intellitrack-signature']
    );
  });
  ...
  it('should use base64url encoding (no +/= characters)', async () => {
    ...
    const result = await signer.sign(body, timestamp);
    const signature = result.headers['x-intellitrack-signature'];
