...
This document describes the deployment process for IntelliTrack SDK packages.
...
- `packages/intellitrack-core/package.json`
- `packages/intellitrack-nextjs/package.json`
...
cd packages/intellitrack-core
pnpm publish --access public

# Then publish framework adapters
cd ../intellitrack-nextjs
pnpm publish --access public
```
...
1. **Verify npm packages**
   ```bash
   npm view @intellitrack/core version
   npm view @intellitrack/nextjs version
   ```

2. **Test installation in fresh project**
   ```bash
   mkdir /tmp/test-intellitrack
   cd /tmp/test-intellitrack
   npm init -y
   npm install @intellitrack/nextjs
   ```

3. **Verify package contents**
   ```bash
   npm pack @intellitrack/core --dry-run
   npm pack @intellitrack/nextjs --dry-run
   ```
...
```bash
npm deprecate @intellitrack/core@1.x.x "Critical bug, please upgrade to 1.x.y"
npm deprecate @intellitrack/nextjs@1.x.x "Critical bug, please upgrade to 1.x.y"
```
...
```bash
npm unpublish @intellitrack/core@1.x.x
```
...
Verify you're publishing to the correct scope (`@intellitrack/`).
