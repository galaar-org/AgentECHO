...
IntelliTrack is a plug-and-play traffic analyzer SDK for capturing request-level analytics without blocking user traffic. This document describes the system architecture and design decisions.

## System Overview

IntelliTrack follows a modular, layered architecture with clear separation of concerns:
...
The main orchestrator that coordinates all components. Created via `createIntelliTrackClient()` factory function.
...
```typescript
const client = createIntelliTrackClient(config);
