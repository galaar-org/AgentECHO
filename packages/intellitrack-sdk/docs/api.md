...
This document describes the public APIs exposed by IntelliTrack packages.

## Authentication

IntelliTrack uses HMAC-SHA256 for payload authentication. All events sent to your backend include:

| Header | Description |
|--------|-------------|
| `x-intellitrack-key-id` | Key identifier for multi-key support |
| `x-intellitrack-ts` | Unix timestamp in milliseconds |
| `x-intellitrack-signature` | Base64url-encoded HMAC-SHA256 signature |
...
### `createIntelliTrackClient(config)`

Creates a new IntelliTrack client instance.

```typescript
import { createIntelliTrackClient } from '@intellitrack/core';

const client = createIntelliTrackClient({
...
**Returns:** `IntelliTrackClient`

### `IntelliTrackClient.captureEvent(eventData)`
...
```typescript
import { createEventBuilder } from '@intellitrack/core';
...
```typescript
import { createSampler } from '@intellitrack/core';
...
```typescript
import { createSigner } from '@intellitrack/core';
...
```typescript
import { createTransport } from '@intellitrack/core';
...
### `createIntelliTrack(config)`

Creates an IntelliTrack instance configured for Next.js.
...
IntelliTrack SDK never throws errors (fire-and-forget). Backend implementations should return:
