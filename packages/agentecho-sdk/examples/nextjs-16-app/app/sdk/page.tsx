...
export default function SDKPage() {
	return (
		<div className="min-h-screen bg-zinc-50">
			...
			<ProseLayout
				title="SDK Reference"
				description="Everything you need to integrate AgentECHO into your Next.js application."
			>
				<H2>Installation</H2>
				<P>Install the SDK using your preferred package manager:</P>
				<CodeBlock filename="Terminal">{`pnpm add @galaar/agentecho-nextjs`}</CodeBlock>

				<H2>Quick Start</H2>
				<P>
					Add AgentECHO to your Next.js application in under 5 minutes. The
					SDK integrates with <Code>proxy.ts</Code> (Next.js 16+) or{" "}
					<Code>middleware.ts</Code> (Next.js 15+).
				</P>

				<H3>1. Create your proxy file</H3>
				<CodeBlock filename="proxy.ts">{`import { createAgentECHO } from '@galaar/agentecho-nextjs';
...
export const config = {
  matcher: ['/((?!_next/static|_next/image|favicon.ico).*)'],
				...
				<CodeBlock filename=".env.local">{`AGENTECHO_INGEST_URL=https://your-backend.com/v1/events
AGENTECHO_KEY_ID=prod-key-1
AGENTECHO_HMAC_SECRET=your-secret`}</CodeBlock>
				...
				<H2>How It Works</H2>
				<P>
					AgentECHO uses a fire-and-forget architecture that never impacts
