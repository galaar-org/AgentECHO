...
AgentECHO is a client-side SDK that sends events to your backend. It does not include a database. This document describes the recommended schema for storing AgentECHO events in your backend.

## Overview

AgentECHO is a fire-and-forget analytics SDK. Your backend is responsible for:
...
Events received from AgentECHO follow this structure:
...
CREATE INDEX idx_events_timestamp ON agentecho_events(timestamp);
CREATE INDEX idx_events_key_id ON agentecho_events(key_id);
CREATE INDEX idx_events_pathname ON agentecho_events(pathname);
CREATE INDEX idx_events_ip ON agentecho_events(ip);
CREATE INDEX idx_events_tags ON agentecho_events USING GIN(tags);
```
...
// Indexes
db.agentecho_events.createIndex({ requestId: 1 }, { unique: true });
db.agentecho_events.createIndex({ timestamp: -1 });
db.agentecho_events.createIndex({ keyId: 1, timestamp: -1 });
db.agentecho_events.createIndex({ pathname: 1 });
```

## Relationships

AgentECHO events are standalone records with no foreign key relationships. However, you may want to join with:
...
CREATE TABLE events_2024_01 PARTITION OF agentecho_events
...
When AgentECHO releases new event fields:
