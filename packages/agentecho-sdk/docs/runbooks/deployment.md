...
This document describes the deployment process for AgentECHO SDK packages.
...
- `packages/agentecho-core/package.json`
- `packages/agentecho-nextjs/package.json`
...
1. **Verify npm packages**
   ```bash
   npm view @galaar/agentecho-core version
   npm view @galaar/agentecho-nextjs version
   ```

2. **Test installation in fresh project**
   ```bash
   mkdir /tmp/test-agentecho
   cd /tmp/test-agentecho
   npm init -y
   npm install @galaar/agentecho-nextjs
   ```

3. **Verify package contents**
   ```bash
   npm pack @galaar/agentecho-core --dry-run
   npm pack @galaar/agentecho-nextjs --dry-run
   ```
...
```bash
npm deprecate @galaar/agentecho-core@1.x.x "Critical bug, please upgrade to 1.x.y"
npm deprecate @galaar/agentecho-nextjs@1.x.x "Critical bug, please upgrade to 1.x.y"
```
...
```bash
npm unpublish @galaar/agentecho-core@1.x.x
```
...
Verify you're publishing to the correct scope (`@galaar/`).
