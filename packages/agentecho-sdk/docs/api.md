...
This document describes the public APIs exposed by AgentECHO packages.

## Authentication

AgentECHO uses HMAC-SHA256 for payload authentication. All events sent to your backend include:

| Header | Description |
|--------|-------------|
| `x-agentecho-key-id` | Key identifier for multi-key support |
| `x-agentecho-ts` | Unix timestamp in milliseconds |
| `x-agentecho-signature` | Base64url-encoded HMAC-SHA256 signature |
...
### `createAgentECHOClient(config)`

Creates a new AgentECHO client instance.

```typescript
import { createAgentECHOClient } from '@galaar/agentecho-core';

const client = createAgentECHOClient({
...
**Returns:** `AgentECHOClient`

### `AgentECHOClient.captureEvent(eventData)`
...
```typescript
import { createEventBuilder } from '@galaar/agentecho-core';
...
```typescript
import { createSampler } from '@galaar/agentecho-core';
...
```typescript
import { createSigner } from '@galaar/agentecho-core';
...
```typescript
import { createTransport } from '@galaar/agentecho-core';
...
### `createAgentECHO(config)`

Creates an AgentECHO instance configured for Next.js.
...
AgentECHO SDK never throws errors (fire-and-forget). Backend implementations should return:
