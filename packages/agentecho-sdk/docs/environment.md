...
This document describes all environment variables and configuration options for AgentECHO.

## Required Environment Variables

| Variable | Description | Example | Required |
|----------|-------------|---------|----------|
| `AGENTECHO_INGEST_URL` | Backend endpoint URL | `https://ingest.example.com/v1/events` | Yes |
| `AGENTECHO_KEY_ID` | Key identifier for signing | `prod-key-1` | Yes |
| `AGENTECHO_HMAC_SECRET` | HMAC secret for signing | `your-32-byte-secret` | Yes |

## Optional Environment Variables

| Variable | Description | Example | Default |
|----------|-------------|---------|---------|
| `AGENTECHO_SAMPLE_RATE` | Sampling rate (0-1) | `0.1` | `0.1` |
| `AGENTECHO_TIMEOUT` | Request timeout in ms | `3000` | `3000` |
| `AGENTECHO_DEBUG` | Enable debug logging | `true` | `false` |
...
1. **Environment files** (development only)
   ...
   AGENTECHO_HMAC_SECRET=your-secret-here
   ```
...
AgentECHO supports multiple key IDs for zero-downtime rotation:
...
```bash
# .env.local
AGENTECHO_INGEST_URL=http://localhost:8080/v1/events
AGENTECHO_KEY_ID=dev-key
AGENTECHO_HMAC_SECRET=dev-secret-not-for-production
AGENTECHO_SAMPLE_RATE=1.0
AGENTECHO_DEBUG=true
```
...
```bash
AGENTECHO_INGEST_URL=https://staging-ingest.example.com/v1/events
AGENTECHO_KEY_ID=staging-key-1
AGENTECHO_HMAC_SECRET=<from-secret-manager>
AGENTECHO_SAMPLE_RATE=0.5
AGENTECHO_DEBUG=true
```
...
```bash
AGENTECHO_INGEST_URL=https://ingest.example.com/v1/events
AGENTECHO_KEY_ID=prod-key-1
AGENTECHO_HMAC_SECRET=<from-secret-manager>
AGENTECHO_SAMPLE_RATE=0.1
AGENTECHO_DEBUG=false
```
...
AGENTECHO_INGEST_URL=https://ingest.example.com/v1/events
AGENTECHO_KEY_ID=prod-key-1
AGENTECHO_HMAC_SECRET=your-secret
```
...
AgentECHO uses Web Crypto API and is fully compatible with:
...
AgentECHO validates configuration at initialization:

```typescript
// Throws if required config is missing
const tracker = createAgentECHO({
  ingestUrl: process.env.AGENTECHO_INGEST_URL!, // Required
  keyId: process.env.AGENTECHO_KEY_ID!,         // Required
  hmacSecret: process.env.AGENTECHO_HMAC_SECRET!, // Required
});
```
...
function validateConfig() {
  const required = [
    'AGENTECHO_INGEST_URL',
    'AGENTECHO_KEY_ID',
    'AGENTECHO_HMAC_SECRET',
  ];
...
### "AGENTECHO_HMAC_SECRET is not defined"
